...
*/
    if (q) {
        for (unsigned i = 0; i < QUEUE_SIZE; ++i) q->arr[i] = '*';
...
*/
    if (queueEmpty(q) || !c) queueError(QERR_FRONT);
    else *c = q->arr[q->f_index];
}
...
*/
    if (queueEmpty(q)) queueError(QERR_REMOVE);
    else q->f_index = nextIndex(q->f_index);
}
...
*/
    if (queueEmpty(q) || !c) queueError(QERR_GET);
    else {
        queueFront(q, c);
        queueRemove(q);
    }
...
*/
    if (queueFull(q)) queueError(QERR_UP);
    else {
        q->arr[q->b_index] = c;
